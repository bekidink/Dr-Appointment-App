...
const ServiceDetail = () => {
  ...
  return (
    <ScrollView className="flex-1 bg-white">
      <Stack.Screen options={{ title: service?.title || 'Service Detail' }} />

      {loading ? (
        <ActivityIndicator size="large" color="#1C2A3A" className="mt-10" />
      ) : (
        <ScrollView className="px-4 pb-6">
          <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 8 }}>All Services</Text>
          <FlatList
            data={service || []}
          ...
          <Text className="mb-2 text-lg font-semibold text-[#1C2A3A]">Available Doctors</Text>
